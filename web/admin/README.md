...
- [ ] Add Google login.
- [ ] Add LinkedIn login.
- [ ] Create a new Supabase migration for `user_roles` table with `user_id` (REFERENCES `auth.users(id)`), `role` (ENUM: 'Admin', 'Creator', 'Viewer').
- [ ] Set up Row Level Security (RLS) for the `user_roles` table to ensure only admins can modify roles.
- [ ] Implement logic to assign roles to users upon registration or by an admin.
- [ ] Enhance `src/middleware/auth-middleware.ts` to fetch user roles from Supabase.
...
- [ ] Create dummy user accounts in Supabase for each role (Admin, Creator, Viewer).
...
- [x] Update `supabase/migrations/01_create_conversations_and_messages_tables.sql` to add `is_public` column to `conversations` table.
...
- [ ] Create a new Supabase migration for a `dashboards` table, including `user_id` (Creator), `title`, and `is_public` (boolean, default false).
...
4.  **`is_public` column in `conversations` table**: Added via modification to `supabase/migrations/01_create_conversations_and_messages_tables.sql`.
