...
export async function getAuthTokens(): Promise<{
  ...
  const cookieStore = await cookies();
  return {
    accessToken: cookieStore.get(AUTH_COOKIE_KEYS.accessToken)?.value,
    refreshToken: cookieStore.get(AUTH_COOKIE_KEYS.refreshToken)?.value,
    tokenType: cookieStore.get(AUTH_COOKIE_KEYS.tokenType)?.value,
    expiresAt: cookieStore.get(AUTH_COOKIE_KEYS.expiresAt)?.value,
  };
}
